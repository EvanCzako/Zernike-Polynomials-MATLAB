function recreation = zernike_recreation(im,indices,desired_indices)
    ...
    a_coeffs = zernike_moments(im,indices);

    recreation = zeros(NumRows,NumCols);   

    for i = 1:length(desired_indices)
        
        recreation = recreation + a_coeffs(desired_indices(i)).*zernikeMats(:,:,desired_indices(i));
